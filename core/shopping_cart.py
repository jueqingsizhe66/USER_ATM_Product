...
def get_goods():
    ...
    :return: goods{dict}
    """
    with open("../db/goods.json", "r", encoding="utf-8") as f:
        goods = json.load(f)
        return goods
...
def set_goods(goods):
    ...
    :return: None
    """
    with open("../db/goods.json", "w", encoding="utf-8") as f:
...
def customer_mode():
    ...
    # 打印相关信息
    print("当前用户信息：")
    print(f'\t当前余额：\033[32;1m \t {balance} \033[0m')
    print("\t购物车：")
    ...
    while True:
        # 打印商品列表
        print("商品列表：")
        for index, value in enumerate(goods):  ## 知识点 enumerate
            print(" ", index, "->", value)
